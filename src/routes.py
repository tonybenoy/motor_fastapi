...
from .utils import create_aliased_response, get_student_ids, get_students_in
...
async def read_students(db: AsyncIOMotorClient = Depends(dependency=get_database)):
    data = []
    async for student in db["sample_training"]["students"].find():
        data.append(student)
...
async def read_student_classes(
    ...
    student = None
    async for doc in db["sample_training"]["students"].aggregate(
        [
...
async def read_student_performance(student_id: int):
    """{
        "student_id": integer,
        "student_name": string,
        "classes": [
...
async def read_class_students(
    ...
    students = await get_student_ids(db=db, class_id=class_id)
    los = await get_students_in(db=db, student_ids=students)
...
@router.get("/class/{class_id}/final-grade-sheet")
async def final_grade_sheet(class_id: str):
    """
    {
            "class_id": integer,
            "students": [
